        ...
        <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
            ...
            <div class="flex justify-between items-center mb-4">
                <div class="flex items-center space-x-2">
                    <!-- Search Bar -->
                    <input wire:model.live="search" type="text" placeholder="Search employees..."
                ...
                <button wire:click="$dispatch('openModal', { component: 'employees.employee-modal'})"
            ...
            <!-- Table -->
            <table id="employeeTable" class="min-w-full bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-md">
                <thead>
                ...
                <tbody>
                    @forelse ($employees as $key => $employee)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition duration-200 ease-in-out">
                            ...
                            <td
                                class="border-b border-gray-200 dark:border-gray-700 px-4 py-3 text-sm text-gray-600 dark:text-gray-300">
                                {{ $employee->department ? $employee->department->name : 'N/A' }}</td>
                            <td
                                class="border-b border-gray-200 dark:border-gray-700 px-4 py-3 text-sm text-gray-600 dark:text-gray-300">
                                {{ \Carbon\Carbon::parse($employee->birthdate)->format('F j, Y') }}</td>
                            <td
                                class="border-b border-gray-200 dark:border-gray-700 px-4 py-3 text-sm text-gray-600 dark:text-gray-300">
                                {{ \Carbon\Carbon::parse($employee->date_hired)->format('F j, Y') }}</td>
                            <td class="border-b border-gray-200 dark:border-gray-700 px-4 py-3 text-center">
                                <button
                                    wire:click="$dispatch('openModal', {component: 'employees.employee-modal', arguments: {employeeId: {{ $employee->id }}, isView: {{ true }}}})"
                                ...
                                <button
                                    wire:click="$dispatch('openModal', {component: 'employees.employee-modal', arguments: {employeeId: {{ $employee->id }}, isEdit: {{ true }}}})"
                                ...
                                <button
                                    wire:click="$dispatch('openModal', { component: 'employees.delete-employee-modal', arguments: { employeeId: {{ $employee->id }}}})"
